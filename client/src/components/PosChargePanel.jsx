...
export default function PosChargePanel({ receipt, quoteTotal, onPaid }) {
  ...
  if (!receipt?._id) {
    return (
      <div className="rounded-sm border border-amber-200 bg-white/70 px-3 py-2 text-[11px] text-amber-900/70">
        Save the quote first, then send the amount to the handheld. Stock only moves after approved (or cash / e-transfer).
      </div>
    );
  }
  ...
  return (
    <div className="rounded-sm border border-amber-300/60 bg-white/80 px-3 py-2.5 space-y-2">
      ...
      <p className="text-[10px] leading-snug text-amber-900/65">
        Send the quote total. The handheld takes tap/insert. This page waits for <span className="font-semibold">APPROVED</span> or <span className="font-semibold">DECLINED</span>, then pulls linked stock only on approval. Cash and e-transfer skip the terminal.
      </p>
