...
function storefrontFilter() {
  return {
    $or: [
      ...
      { status: "sold", showOnStorefrontWhenEmpty: true },
    ],
  };
}
...
async function applyChange({ item, newQuantity, newStatus, reason, actor, extra = {} }) {
  ...
  const status = canonicalStockStatus(newStatus || statusAfterQty(item, qty));
  ...
  item.quantity = qty;
  item.status = canonicalStockStatus(status);
  ...
  if (item.status === "in-stock" && (old.status === "sold" || old.status === "out-of-stock") && !extra.soldTo && !extra.dateSold) {
    item.soldTo = "";
...
async function applySaleFromReceipt(receipt, actor) {
  ...
  for (const line of receipt.items || []) {
    ...
    item.reservedQty = Math.max(0, (Number(item.reservedQty) || 0) - saleQty);
    // eslint-disable-next-line no-await-in-loop
...
function matchScore(query, name) {
  ...
  if (tokens.length === 1 && words.some((w) => w.startsWith(q))) return 220;
  const allWordStarts = tokens.every((t) => words.some((w) => w.startsWith(t)));
  if (allWordStarts) return 200;
  ...
  if (query.length >= 1) {
    ...
    for (const model of catalogModels()) {
      ...
      push(
        ...
        score - familyRank(model.family) * 8,
        0
      );
    }
  }
